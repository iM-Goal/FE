...
export default function DepositDetailScreen({ navigation }: any) {
    // 📋 미션 리스트 데이터 (완료된 미션 추가)
    const missionList = [
        { id: '1', name: '배달 앱 대신 집밥 해먹기 🍳', amount: '20,000원', dDay: 'D-2', date: '2026.07.09 만기', status: '진행 중' },
        { id: '2', name: '커피는 하루에 딱 1잔만 ☕', amount: '20,000원', dDay: 'D-2', date: '2026.07.09 만기', status: '진행 중' },
        { id: '3', name: '택시 대신 지하철, 버스 도전하기 🚇', amount: '15,000원', dDay: '완료', date: '2026.07.05 해제', status: '해제 완료' }, // 🆕 완료된 미션 추가
    ];
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <LinearGradient colors={['#004B87', '#009D8B']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.summaryCard}>
                    ...
                    <Text style={styles.summaryAmount}>40,000 원</Text>
                        ...
                        return (
                            <View key={item.id} style={styles.timelineItem}>
                                ...
                                <View style={styles.lineColumn}>
                                    <View style={[
                                        styles.timelineDot,
                                        isCompleted && { backgroundColor: '#009D8B', borderColor: '#E6F6F4' }
                                    ]}>
                                ...
                                <View style={[styles.infoCard, isCompleted && styles.completedInfoCard]}>
                                    <View style={styles.cardHeader}>
                                        <Text style={[styles.dDayText, isCompleted && { color: '#009D8B' }]}>{item.dDay}</Text>
                                        <View style={[
                                            styles.statusBadge,
...
const styles = StyleSheet.create({
    ...
    timelineDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#FF5E62', borderWidth: 2, borderColor: '#FFFFFF', zIndex: 2, justifyContent: 'center', alignItems: 'center' },
    ...
    completedInfoCard: { backgroundColor: '#F9FAFB', borderColor: '#E5E7EB', opacity: 0.8 }, // 완료 시 약간 딤드(dimmed) 처리하여 가독성 분리

    cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    dDayText: { fontSize: 18, fontWeight: '700', color: '#FF5E62' },
